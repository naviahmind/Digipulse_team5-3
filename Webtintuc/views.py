...
def index(request):
    ...
    homepage_ads = Advertisement.objects.filter(
        position='homepage',
        is_active=True,
    ).filter(
...
def edit_post(request, slug):
    post = get_object_or_404(Post, slug=slug)
    # Tác giả chỉ được sửa bài của mình; admin sửa tất cả
    if not (is_author(request.user) and (post.author == request.user or is_admin_role(request.user) or request.user.is_staff)):
        return redirect('webtintuc:index')
    if request.method == 'POST':
...
def manage_users(request):
    ...
    if role_filter == 'admin':
        ...
        users = users.filter(profile__role='author')
    elif role_filter == 'member':
        users = users.filter(is_superuser=False, profile__role='user')
